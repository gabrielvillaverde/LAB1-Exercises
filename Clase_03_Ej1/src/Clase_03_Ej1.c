/*
 ============================================================================
 Hacer un programa que pida un número al usuario, después que pida una operación (+, -, /, *),
 después que pida otro número. Según la operación ingresada, se llamará a una función
 que ejecute la operación sumar(), restar(), multiplicar() o dividir().
 Y luego mostrar al usuario el resultado. El mostrar resultado no puede estar dentro de
 las funciones que se crean para hacer la operación. En la función dividir, validar no dividir por cero.
...
int main(void) {
	...
	printf("Ingrese una operación: (+, -, * o /)\n ");
	fflush(stdin); // Solo en Windows
	// Si es Linux = __fpurge(stdin);
	scanf("%c", &operacion);
...
// Declaración de funciones, (antes de ver el tema de punteros y bibliotecas)
int sumar(int operador1,int operador2){
	int resultado;
