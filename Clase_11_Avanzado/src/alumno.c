...
int alumno_addAlumno(Alumno* pArrayList, int len, int index)
{
	...
	if(pArrayList != NULL && len > 0 && index >= 0 && index <= len && pArrayList[index].isEmpty == TRUE)
	{
		if
		(
		utn_getName("\nName of alumno:\n","Error, please type a correct name.\n",bufferAlumno.name,2,LONG_NAME)==0 // Me tiraba error porque estaba usando &bufferAlumno, ¿por qué acá no y en las otras sí el &?
			&&
		utn_getName("\nLast name of alumno:\n","Error, please type a correct last name.\n",bufferAlumno.lastName,2,LONG_LASTNAME)==0 // Me tiraba error porque estaba usando &bufferAlumno, ¿por qué acá no y en las otras sí el &?
			&&
