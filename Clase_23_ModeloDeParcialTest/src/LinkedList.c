 ...
 *
 */
// Explicación: Clase 19/11 (HORA: 08:15)
// Aplicación: HORA: 08:25
// Atento: 8:38
...
{	// Explicación: Hora 8:58 (clase 23), pero lo hace con el remove
	...
	if(this != NULL && pFunc != NULL)
	{
		...
		if(pFilteredList != NULL)
		{
			for(int i = 0 ; i < len ; i++)
			{
				pElement = ll_get(this, i); // Clase 19/11 (HORA: 08:32) Realizo un recorrido de la lista original.
				if(pFunc(pElement)) // Si la función genérica devuelve TRUE... HORA 07:00 (Clase 19/11)
				{
    ...
    return pFilteredList; // Retorno la lista filtrada.
}

// Explicación: HORA 07:26 (Clase 19/11)
// HORA: 07:48 (Clase 19/11), hacer genérico el último argumento a través de la creación de una estructura.
// Resolverlo sin usar estructura: 07:57
LinkedList* ll_filterWithOneMoreArgument(LinkedList* this, int (*pFunc)(void*, char*), char* atributoDeseado)
{
	LinkedList* pFilteredList = NULL;
	void* pElement;
    int len = ll_len(this);

	if(this != NULL && pFunc != NULL && atributoDeseado != NULL)
	{
		pFilteredList = ll_newLinkedList(); // Devuelvo una lista nueva
		if(pFilteredList != NULL)
		{
			for(int i = 0 ; i < len ; i++)
			{
				...
				if(pFunc(pElement, atributoDeseado)) // Si la función devuelve TRUE...
				{
					ll_add(pFilteredList, pElement); // Agrego el elemento.
				}
			}
		}
	}
...
/* Argumentos variables. HORA: 06:55. Para solucionar lo de agregar argumentos de más al filter y que sea genérico.
 * No vamos a ver este tema.
 * Hora: 07:35 (Clase 19/11) Para que ll_filter siga siendo genérico, debería generar una estructura:
 *
 */
